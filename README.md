...
### Topside
Dyver Topside (target `Dyver`) is your go-to command center for all of your ROVs. It can recieve telemetry, send instructions, and stream camera data from any robot running the Dyver Robot. 

### Robot
Dyver Robot will communicate with topside, manage sensors, and perform general flight controller tasks. It is important to note that the Dyver Robot application **is not a driver for a flight controller.** It is designed to run on what would traditionally be a companion computer, and uses an external IMU.
